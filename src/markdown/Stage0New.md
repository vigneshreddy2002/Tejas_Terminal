...
1. (`ncn-m001#`) Install the latest `docs-csm` RPM.
   ...
   > Otherwise, use the following procedure to download and install the latest documentation.
      ...
      >> Download the latest `docs-csm` RPM to an external system and copy it to `ncn-m001`.
   ...
   1. Check if the `rbd` device already exists.
      ...
      <Code >
       Expected output if `rbd` device does not exist:
         ...
         <div className = "text">             
          _Pool csm_admin_pool does not exist_        
...
> Skip the `Manual copy` subsection and proceed to [Stage 0.2 - Prerequisites](#stage-02---prerequisites)
